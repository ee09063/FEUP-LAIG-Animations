...
class Primitive
{
	...
	public:
		...
		vector<float*> getCtrlPoints(){return this->ctrlpoints;}
		void addCtrlPoint(float* ctrlpoint){
			this->ctrlpoints.push_back(ctrlpoint);
		}
		int getNumControlPoints(){return this->ctrlpoints.size();}

		/*FOR FLAG*/
		virtual void updateShaderTime(float time)=0;
		virtual void updateShaderWind(float wind)=0;
};
...
class Node
{
	...
	public:
		float m[4][4];
		Node(char* id, Appearance* app, char* appMode, bool displayList)
		{
			this->id=id;
			this->app=app;
			this->visited = false;
			...
			if(displayList) displayListId = glGenLists(1);
		...
		bool getDisplayList(){return this->displayList;}
		GLint getDisplayListId(){return this->displayListId;}
		/*
		...
		*/
		...
		void addAnimations(vector<Animation*> animations){this->animations = animations;}
		Animation* getAnimation(int n){return this->animations.at(n);}
		unsigned long nodeTime;
		float nodeX;
		float nodeY;
		float nodeZ;
		float nodeA;
