		...
		if(graphElement)
		{
			...
			for(TiXmlElement* node = graphElement->FirstChildElement();node;node=node->NextSiblingElement())
			{
				...
				if(node->Attribute("displaylist")){
					char* dl = (char*)node->Attribute("displaylist");
					if(strcmp(dl, "true") == 0) disList = true;
					else if(strcmp(dl, "false") == 0) disList = false;
					else{
						printf("ERROR: INVALID VALUE IN DISPLAYLIST IN NODE '%s'", id);
						getchar();
						exit(-1);
					}
				}
				...
				/*ANIMATION*/
				vector<Animation*> nodeAnime;
				TiXmlElement* animations = node->FirstChildElement("animations");
				if(animations)
				{
					for(TiXmlElement* nodeAnimation = animations->FirstChildElement();
						nodeAnimation;
						nodeAnimation = nodeAnimation->NextSiblingElement())
					{
						char* animationRef;
						Animation* animation = NULL;
						animationRef = (char*)nodeAnimation->Attribute("id");
						animation = this->getAnimationById(animationRef);

						nodeAnime.push_back(animation);
					}
				}
				
				Node* n = new Node(id, appearance, appearanceRef, disList);
				
				//n->setAnimation(animation);

				n->addAnimations(nodeAnime);
				...
				if(primitives)
				{
					...
					for(prim = primitives->FirstChildElement(); prim; prim = prim->NextSiblingElement())
					{
						...
						/*REGISTER A PLANE*/
						else if(strcmp(prim->Value(), "plane")==0){
							p = registerPlane(prim, id);
						}
						/*REGISTER A PATCH*/
						else if(strcmp(prim->Value(), "patch")==0){
							p = registerPatch(prim, id);
						}
						/*REGISTER A FLAG*/
						else if(strcmp("flag", prim->Value())==0)
						{
							p = registerFlag(prim, id);
						}
						/*REGISTER A VEHICLE*/
						else if(strcmp("vehicle", prim->Value())==0)
						{
							p = registerVehicle(prim, id);
						}
						else{
...
Primitive* XMLScene::registerRectangle(TiXmlElement* prim, char* id)
{
...
Primitive* XMLScene::registerTriangle(TiXmlElement* prim, char* id)
{
...
Primitive* XMLScene::registerCylinder(TiXmlElement* prim, char* id)
{
...
Primitive* XMLScene::registerSphere(TiXmlElement* prim, char* id)
{
...
Primitive* XMLScene::registerTorus(TiXmlElement* prim, char* id)
{
...
Primitive* XMLScene::registerPlane(TiXmlElement* prim, char* id)
{
	Primitive* p = new Plane();
	float numDiv;
	vector<float> f;
	if(prim->QueryFloatAttribute("parts",&numDiv)==TIXML_SUCCESS){
		f.push_back(numDiv);
		p->setValues(f);
		return p;
	}
	else{
		printf("ERROR PARSING PLANE PRIMITVE IN NODE '%s'. EXITING...\n", id);
		getchar();
		exit(-1);
	}
	return NULL;
...
Primitive* XMLScene::registerPatch(TiXmlElement* prim, char* id)
{
	Primitive* p = new Patch();
	vector<float>f;
	float order, partsU, partsV;
	
	if(prim->QueryFloatAttribute("order",&order)==TIXML_SUCCESS &&
		 prim->QueryFloatAttribute("partsU",&partsU)==TIXML_SUCCESS &&
		 prim->QueryFloatAttribute("partsV",&partsV)==TIXML_SUCCESS)
	{
		f.push_back(order);
		f.push_back(partsU);
		f.push_back(partsV);
		...
		printf("ERROR PARSING PATCH PRIMITIVE IN NODE %s. EXITING...\n", id);
		getchar();
		exit(-1);
	}

	if(prim->Attribute("compute")){
		char* compute = (char*)prim->Attribute("compute");

		if(strcmp("fill", compute)==0){f.push_back(0);}
		else if(strcmp("line", compute)==0){f.push_back(1);}
		else if(strcmp("point", compute)==0){f.push_back(2);}
		else{
			printf("INVALID VALUE IN COMPUTE ATTRIBUTE IN NODE %s. EXITING...\n",id);
			getchar();
			exit(-1);
		}
		p->setValues(f);
		...
		printf("COMPUTE ATTRIBUTE MISSING IN NODE %s. EXITING...\n", id);
		getchar();
		exit(-1);
	}
	float ctrlpCount = 0;
	for(TiXmlElement* ctrlp = prim->FirstChildElement("controlpoint"); ctrlp; ctrlp = ctrlp->NextSiblingElement())
	{
		float* point = (float*)malloc(sizeof(float)*3);
		if(ctrlp->QueryFloatAttribute("x",&point[0])==TIXML_SUCCESS &&
			ctrlp->QueryFloatAttribute("y",&point[1])==TIXML_SUCCESS &&
			ctrlp->QueryFloatAttribute("z",&point[2])==TIXML_SUCCESS)
		{
				p->addCtrlPoint(point);
				ctrlpCount++;
		}
		else{
			printf("ERROR PARSING CONTROL POINTS IN NODE %s. EXITING...\n", id);
			getchar();
			exit(-1);
		}
	}
	if(ctrlpCount != pow((order+1),2)){
		printf("WRONG NUMBER OF CONTROL POINTS IN NODE %s. %d REQUIRED. %d PRESENT. EXITING...\n", id, (int)pow((order+1),2), (int)ctrlpCount);
		getchar();
		exit(-1);
	}
	else
...
Primitive* XMLScene::registerFlag(TiXmlElement* prim, char* id)
{
	char* textureFile;
	if(prim->Attribute("texture"))
	{
		textureFile = (char*)prim->Attribute("texture");
	}
	else{
		printf("ERROR: NO TEXTURE FILE IN FLAG IN NODE '%s'\n", id);
		getchar();
		exit(-1);
	}
	Primitive* p = new Flag(textureFile);
	return p;
}

Primitive* XMLScene::registerVehicle(TiXmlElement* prim, char* id)
{
...
void XMLScene::registerAnimations(TiXmlElement* animationElement)
{
	...
	{	
		...
		if(strcmp("linear", animationType)==0)
		{
			...
			int _count = 0;
			for(TiXmlElement* ctrlp = anime->FirstChildElement(); ctrlp; ctrlp=ctrlp->NextSiblingElement())
			{
				_count++;
				float* ctrlpoint = (float*)malloc(sizeof(float)*3);
				float x, y, z;
				if(ctrlp->QueryFloatAttribute("xx",&x)==TIXML_SUCCESS &&
				   ctrlp->QueryFloatAttribute("yy",&y)==TIXML_SUCCESS &&
				   ctrlp->QueryFloatAttribute("zz",&z)==TIXML_SUCCESS){
						ctrlpoint[0] = x;
						ctrlpoint[1] = y;
						ctrlpoint[2] = z;
						animation->addCtrlPoint(ctrlpoint);
				}
				else{
					printf("ERROR PARSING CONTROL %d POINT IN ANIMATION '%s'. EXITING...\n", _count,id);
					getchar();
					exit(-1);
				}
			}
			animation->calculateControlVectors();
			animation->calculateDistances();
			animation->calculateAngles();
			animation->calculateTimeFrags();
			animation->calculateVelocities();
		}
